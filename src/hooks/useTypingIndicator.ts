...
export function useTypingIndicator(otherUserId:string|undefined){
 const {userId}=useAuth(); const [otherTyping,setOtherTyping]=useState(false); const channelRef=useRef<any>(null); const clearRef=useRef<number|undefined>(undefined);
