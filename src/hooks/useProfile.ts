...
function normalizeProfileLocation(profile: Profile | null): Profile | null {
  ...
  if (!location || !country) return profile;

  const escapedCountry = country.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const countryAtEnd = new RegExp(`(?:\\s*[,·-]\\s*|\\s+)${escapedCountry}$`, "i");
  const exactCountry = new RegExp(`^${escapedCountry}$`, "i");
  const cleanedLocation = exactCountry.test(location)
    ? null
    : location.replace(countryAtEnd, "").trim().replace(/[,·-]+$/, "").trim() || null;

  return cleanedLocation === profile.location ? profile : { ...profile, location: cleanedLocation };
}
