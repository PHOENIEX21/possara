...
export function useProfileById(id: string | undefined) {
  return useQuery({ queryKey: ["profile-by-id", id], enabled: !!id, queryFn: async (): Promise<Profile | null> => { const { data, error } = await supabase.from("profiles").select("*").eq("id", id as string).single(); if (error) throw error; return data; } });
}
