...
export interface OpportunityWithOrg extends Opportunity {
  ...
  opportunity_candidates: { opportunity_sources: { name:string; verified_source:boolean; trust_tier:string } | null } | null;
}
...
export function useOpportunities({ limit = 20, categorySlug, categorySlugs }: UseOpportunitiesOptions = {}) {
  return useQuery({
    ...
    queryFn: async (): Promise<OpportunityWithOrg[]> => {
      ...
      let query = supabase.from("opportunities").select("*, organizations(name, verified, is_sponsored), opportunity_categories(name, icon, color), opportunity_candidates(opportunity_sources(name,verified_source,trust_tier))").eq("status", "active").or(`deadline.is.null,deadline.gte.${nowIso}`).order("created_at", { ascending: false }).limit(limit);
