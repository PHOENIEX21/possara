...
export function useApplyToJob(jobId:string){
 const {userId}=useAuth();const qc=useQueryClient();
 return useMutation({mutationFn:async(input:{cvUrl:string;coverLetter:string;answers:{questionId:string;value:string}[]})=>{if(!userId)throw new Error("Sign in first.");const {data,error}=await supabase.rpc("submit_job_application",{p_job_id:jobId,p_cv_url:input.cvUrl,p_cover_letter:input.coverLetter,p_answers:input.answers});if(error)throw error;return {id:String(data)};},onSuccess:()=>qc.invalidateQueries({queryKey:["my-job-applications"]})});
}
