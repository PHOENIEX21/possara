...
export function useRegisterOrganization(){
 ...
 return useMutation({mutationFn:async(input:{name:string;category:string;location:string;description:string;logoUrl?:string;applicationDefaults?:ApplicationDefaults})=>{if(!userId)throw new Error("Sign in first.");const slugBase=input.name.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"").slice(0,48)||"organization";const slug=`${slugBase}-${crypto.randomUUID().slice(0,6)}`;const handle=(slugBase.replace(/-/g,"").slice(0,22)||"org")+crypto.randomUUID().replace(/-/g,"").slice(0,6);const {data,error}=await supabase.from("organizations").insert({owner_id:userId,name:input.name.trim(),slug,handle,category:input.category,industry:input.category,location:input.location,headquarters:input.location,description:input.description.trim(),logo_url:input.logoUrl||null,application_defaults:input.applicationDefaults,verification_status:"pending",verified:false}).select("*").single();if(error)throw error;return data;},onSuccess:()=>qc.invalidateQueries({queryKey:["my-organizations"]})});
}
export function useCreateJob(){
 const {userId}=useAuth();const qc=useQueryClient();
 return useMutation({mutationFn:async(input:{requestId:string;organizationId:string;title:string;roleType:string;rank:string;employmentType:string;workStyle:string;location:string;payMin?:number;payMax?:number;description:string;requirements:string[];requiresCbt:boolean;cbtTimeLimitSeconds?:number;applicationDocumentLabel?:string;applicationDocumentLabels?:string[];jobMediaUrls?:string[];applicationDocumentRequired?:boolean;applicationDocumentAccept?:string[];passportPhotoRequired?:boolean;coverLetterRequired?:boolean;publish?:boolean;closesAt?:string;screening:{questionText:string;answerType:"short_text"|"number"|"date"|"yes_no"|"single_choice";choices?:string[];required:boolean}[]})=>{if(!userId)throw new Error("Sign in first.");const {data,error}=await supabase.rpc("create_job_with_screening",{p_request_id:input.requestId,p_job:{organization_id:input.organizationId,title:input.title.trim(),role_type:input.roleType.trim(),rank:input.rank,employment_type:input.employmentType,work_style:input.workStyle,location:input.location.trim(),pay_min:input.payMin??null,pay_max:input.payMax??null,description:input.description.trim(),requirements:input.requirements,requires_cbt:input.requiresCbt,cbt_time_limit_seconds:input.cbtTimeLimitSeconds??900,application_document_label:input.applicationDocumentLabel??"CV / résumé",application_document_labels:input.applicationDocumentLabels?.filter(Boolean)??[input.applicationDocumentLabel??"CV / résumé"],job_media_urls:input.jobMediaUrls??[],application_document_required:input.applicationDocumentRequired??true,application_document_accept:input.applicationDocumentAccept??["application/pdf","application/msword","application/vnd.openxmlformats-officedocument.wordprocessingml.document","image/jpeg","image/png","image/webp"],passport_photo_required:input.passportPhotoRequired??false,cover_letter_required:input.coverLetterRequired??false,status:input.publish?"open":"draft",posted_by:userId,closes_at:input.closesAt||null},p_questions:input.screening});if(error)throw error;return {...data,published:input.publish===true};},onSuccess:(_,v)=>qc.invalidateQueries({queryKey:["organization-jobs",v.organizationId]})});
}
...
export function useUpdateJobApplication(jobId:string){
 ...
 return useMutation({mutationFn:async(input:{id:string;status:"shortlisted"|"rejected"|"hired"})=>{const {error}=await supabase.from("job_applications").update({status:input.status}).eq("id",input.id);if(error)throw error;},onSuccess:()=>{qc.invalidateQueries({queryKey:["job-applicants",jobId]});qc.invalidateQueries({queryKey:["application-review"]});qc.invalidateQueries({queryKey:["my-job-applications"]});}});
}
