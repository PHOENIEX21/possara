...
function EntryExperience(){
 const [transition,setTransition]=useState<"signup"|"logout"|null>(()=>{
  const value=sessionStorage.getItem("possara-transition");
  return value==="signup"||value==="logout"?value:null;
 });
 ...
 useEffect(()=>{
  ...
  const closeTimer=window.setTimeout(()=>setTransition(null),2750);
 ...
 return <div className={`fixed inset-0 z-[120] flex items-center justify-center overflow-hidden bg-[#11101a] px-6 text-white transition-opacity duration-700 ${leaving?"opacity-0":"opacity-100"}`} role="status" aria-live="polite">
  <style>{`@keyframes possaraFloat{0%,100%{transform:translate3d(0,2px,0) rotate(-1.5deg)}50%{transform:translate3d(0,-17px,0) rotate(1.5deg)}}@keyframes possaraOrbit{from{transform:rotate(0deg) translateX(67px) rotate(0deg)}to{transform:rotate(360deg) translateX(67px) rotate(-360deg)}}@keyframes possaraWord{0%{opacity:0;transform:translateY(16px);letter-spacing:.38em}100%{opacity:1;transform:translateY(0);letter-spacing:.14em}}@keyframes possaraReveal{0%{opacity:0;transform:translateY(13px)}100%{opacity:1;transform:translateY(0)}}`}</style>
  <div className="pointer-events-none absolute inset-0"><div className="absolute left-1/2 top-1/2 h-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-500/10 blur-3xl"/><div className="absolute left-1/2 top-1/2 h-2 w-2 rounded-full bg-white/80 shadow-[0_0_22px_rgba(255,255,255,.65)]" style={{animation:"possaraOrbit 5s linear infinite"}}/></div>
  <div className="relative flex w-full max-w-md flex-col items-center text-center">
   ...
   <p className="mt-7 text-[11px] font-semibold uppercase text-white/45" style={{animation:"possaraWord 1.05s .1s both"}}>POSSARA</p>
   <div style={{animation:"possaraReveal .8s .5s both"}}>
    <h1 className="mt-3 text-2xl font-bold leading-tight sm:text-3xl">{signup?"Welcome to POSSARA.":"See you soon."}</h1>
    <p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-white/60">{signup?"Your new space for people, growth, learning and opportunities is ready.":"Your POSSARA journey will be here when you return."}</p>
   </div>
  </div>
 </div>;
}
