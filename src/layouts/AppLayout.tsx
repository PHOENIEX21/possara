...
function LinkRow({to,label,Icon,onClick}:{to:string;label:string;Icon:typeof HomeIcon;onClick?:()=>void}){const location=useLocation();function activate(){if(to==="/"&&location.pathname==="/"){document.getElementById("home-posts")?.scrollIntoView({behavior:"smooth",block:"start"});}else if(location.pathname===to)window.scrollTo({top:0,behavior:"smooth"});onClick?.();}return <NavLink to={to} end={to==="/"} onClick={activate} className={({isActive})=>`nav-row ${isActive?"nav-row-active":""}`}><Icon size={19}/><span>{label}</span></NavLink>}

function OrganizationSwitcher({close}:{close?:()=>void}){const {data}=useMyOrganizations();const orgs=(data??[]).map((x:any)=>x.organizations).filter(Boolean);if(!orgs.length)return <div className="rounded-2xl bg-brand-light/60 p-2"><p className="px-2 pb-1 text-xs font-semibold text-brand-dark">Hiring tools</p><p className="px-2 pb-2 text-[11px] leading-4 text-ink-light">Create an organization page to post jobs, manage applicants, set timed CBTs and prepare interviews.</p><NavLink to="/organizations/register" onClick={close} className="nav-row"><Briefcase size={19}/><span>Create organization page</span></NavLink></div>;return <div><p className="drawer-label">Work as organization</p>{orgs.map((org:any)=><NavLink key={org.id} to="/organizations/manage" onClick={()=>{localStorage.setItem("possara-active-organization",org.id);close?.();}} className="nav-row">{org.logo_url?<img src={org.logo_url} alt="" className="h-5 w-5 rounded-md object-cover"/>:<Briefcase size={19}/>}<span className="truncate">{org.name}</span></NavLink>)}</div>}
