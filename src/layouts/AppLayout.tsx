...
import { Search, Bell, User, LogOut, Menu, X, Home as HomeIcon, Users, Bookmark, Compass, MessageCircle, Plus, Briefcase, GraduationCap, School, Settings as SettingsIcon, BookOpen, Sparkles, ShieldCheck } from "lucide-react";
...
function EntryExperience(){
 ...
 useEffect(()=>{
  ...
  if(userId){
   const key="possara-signed-in-entry-seen";
   if(sessionStorage.getItem(key))return;
   sessionStorage.setItem(key,"1");
   setVisible(true);
   const leaveTimer=window.setTimeout(()=>setLeaving(true),2600);
   const closeTimer=window.setTimeout(()=>setVisible(false),3300);
   return()=>{window.clearTimeout(leaveTimer);window.clearTimeout(closeTimer)};
  }

  if(sessionStorage.getItem("possara-guest-welcome-dismissed"))return;
  setVisible(true);
 },[loading,userId]);

 function enterAsGuest(){
  sessionStorage.setItem("possara-guest-welcome-dismissed","1");
  setLeaving(true);
 ...
 return <div className={`fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-[#11101a] px-6 text-white transition-opacity duration-700 ${leaving?"opacity-0":"opacity-100"}`} role="dialog" aria-modal="true" aria-label={guest?"Welcome to POSSARA":"Opening POSSARA"}>
  ...
  <div className="pointer-events-none absolute inset-0">
   <div className="absolute left-1/2 top-1/2 h-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-500/10 blur-3xl" style={{animation:"possaraGlow 3.4s ease-in-out infinite"}}/>
   <div className="absolute left-1/2 top-1/2 h-2 w-2 rounded-full bg-white/80 shadow-[0_0_22px_rgba(255,255,255,.65)]" style={{animation:"possaraOrbit 5s linear infinite"}}/>
   <div className="absolute left-[22%] top-[27%] h-1.5 w-1.5 rounded-full bg-white/35"/><div className="absolute right-[19%] top-[34%] h-2 w-2 rounded-full bg-violet-300/45"/><div className="absolute bottom-[23%] left-[30%] h-1 w-1 rounded-full bg-white/45"/>
  </div>
  <div className="relative flex w-full max-w-md flex-col items-center text-center">
   <div className="flex h-28 w-28 items-center justify-center rounded-[2.2rem] border border-white/10 bg-white/[.06] shadow-2xl backdrop-blur-md" style={{animation:"possaraFloat 3.4s ease-in-out infinite"}}><BrandMark className="h-[72px] w-[72px] text-white"/></div>
   <p className="mt-7 text-[11px] font-semibold uppercase text-white/45" style={{animation:"possaraWord 1.15s .15s both"}}>POSSARA</p>
   <div style={{animation:"possaraReveal .9s .65s both"}}>
    <h1 className="mt-3 text-2xl font-bold leading-tight sm:text-3xl">{guest?"Welcome to a world of possibilities.":"Welcome back."}</h1>
    <p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-white/60">{guest?"Discover people, opportunities, learning and ideas that can move you forward.":"Your people, opportunities and progress are ready."}</p>
   </div>
   {guest?<div className="mt-8 w-full space-y-3" style={{animation:"possaraReveal .9s 1.05s both"}}>
     <NavLink to="/signin" onClick={enterAsGuest} className="flex w-full items-center justify-center rounded-full bg-white px-5 py-3 text-sm font-bold text-[#171128] shadow-xl transition hover:-translate-y-0.5">Sign in</NavLink>
     <NavLink to="/signin?mode=signup" onClick={enterAsGuest} className="flex w-full items-center justify-center rounded-full border border-white/20 bg-white/[.06] px-5 py-3 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/[.1]">Join POSSARA</NavLink>
     <button type="button" onClick={enterAsGuest} className="px-4 py-2 text-xs font-medium text-white/55 hover:text-white">Explore first</button>
    </div>:<div className="mt-7 flex items-center gap-2 text-xs font-medium text-white/45" style={{animation:"possaraReveal .8s 1s both"}}><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-violet-300"/>Opening your POSSARA…</div>}
  </div>
 </div>;
}
...
export function AppLayout(){
 ...
 return <div className="min-h-screen bg-paper">
  ...
  {open&&<div className="mobile-drawer"><div className="mobile-drawer-card overflow-y-auto pb-24">{PRIMARY.map(i=><LinkRow key={i.to} to={i.to} label={i.label} Icon={i.icon} onClick={()=>setOpen(false)}/>)}<p className="drawer-label">Learn & grow</p><LinkRow to="/study" label="POSSARA Study" Icon={BookOpen} onClick={()=>setOpen(false)}/><p className="drawer-label">Explore opportunities</p>{OPPS.map(i=><LinkRow key={i.to} to={i.to} label={i.label} Icon={i.icon} onClick={()=>setOpen(false)}/>)}{userId&&<><p className="drawer-label">Your account</p>{isAdmin&&<LinkRow to="/admin" label="Admin" Icon={ShieldCheck} onClick={()=>setOpen(false)}/>}<LinkRow to="/messages" label="Messages" Icon={MessageCircle} onClick={()=>setOpen(false)}/><LinkRow to="/notifications" label="Notifications" Icon={Bell} onClick={()=>setOpen(false)}/><LinkRow to="/saved" label="Saved" Icon={Bookmark} onClick={()=>setOpen(false)}/><LinkRow to="/settings" label="Settings" Icon={SettingsIcon} onClick={()=>setOpen(false)}/><button onClick={logout} className="nav-row w-full"><LogOut size={19}/>Sign out</button></>}</div></div>}
  <div className="app-shell"><aside className="desktop-sidebar"><nav>{PRIMARY.slice(0,3).map(i=><LinkRow key={i.to} to={i.to} label={i.label} Icon={i.icon}/>)}</nav><p className="drawer-label">Learn & grow</p><nav><LinkRow to="/study" label="POSSARA Study" Icon={BookOpen}/></nav><p className="drawer-label">Opportunities</p><nav>{OPPS.map(i=><LinkRow key={i.to} to={i.to} label={i.label} Icon={i.icon}/>)}</nav><div className="sidebar-divider"/>{userId?<>{isAdmin&&<LinkRow to="/admin" label="Admin" Icon={ShieldCheck}/>}<LinkRow to="/saved" label="Saved" Icon={Bookmark}/><LinkRow to="/messages" label="Messages" Icon={MessageCircle}/><LinkRow to="/profile/me" label="Profile" Icon={User}/><LinkRow to="/settings" label="Settings" Icon={SettingsIcon}/><button onClick={logout} className="nav-row mt-1 w-full"><LogOut size={19}/>Sign out</button></>:<NavLink to="/signin" className="sidebar-cta">Join POSSARA</NavLink>}<p className="sidebar-note">Be inspired. Learn. Find opportunities.<br/>Connect. Move forward.</p></aside><main className="app-main"><Outlet/></main></div>
