...
export function AppLayout(){
 ...
 return <div className="min-h-screen bg-paper pt-[60px] sm:pt-16">
  ...
  <nav className={`bottom-nav ${isMessageThread?"hidden":""}`}>{PRIMARY.slice(0,2).map(i=><LinkRow key={i.to} to={i.to} label={i.label} Icon={i.icon}/>)}<button type="button" onClick={create} className="create-fab" aria-label={localStorage.getItem("possara-active-organization")?"Create as organization":"Create post"}><Plus size={24}/></button>{PRIMARY.slice(2).map(i=><LinkRow key={i.to} to={i.to} label={i.label} Icon={i.icon}/>)}</nav>
 </div>
}
