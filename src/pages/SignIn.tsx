...
function friendlyAuthError(message: string) {
  ...
  if (
    ...
    return "POSSARA email delivery is temporarily unavailable. Please try again later.";
  }
...
export function SignIn() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "signup") {
      ...
      const { data: directSignup, error: directSignupError } = await supabase.functions.invoke(
        "signup-direct",
        {
          body: {
            email: cleanEmail,
      ...
      if (!directSignup?.ok) {
        ...
        setError(
          typeof directSignup?.error === "string"
            ? directSignup.error
            : "POSSARA could not create the account right now.",
        );
        return;
      }
      ...
      if (signInError || !signInData.session?.user || !signInData.user) {
        setLoading(false);
        setError(
          signInError
            ? friendlyAuthError(signInError.message)
            : "Account created, but POSSARA could not start your session. Please sign in.",
        );
        return;
      }
      ...
      if (
        verifyError ||
        !verified.user ||
        verified.user.id !== signInData.user.id ||
        verifiedEmail !== cleanEmail
      ) {
        await supabase.auth.signOut({ scope: "local" });
        queryClient.clear();
        setLoading(false);
        setError("POSSARA could not verify the new session. Please sign in with your email and password.");
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-sm rounded-3xl border border-black/[.06] bg-white p-6 shadow-card">
      <div className="mb-5 flex items-center gap-2">
        <BrandMark className="h-9 w-9" />
        <span className="font-bold">POSSARA</span>
      </div>

      {isBanned && (
        <div className="mb-4 rounded-lg bg-flag-light px-4 py-3 text-sm text-flag-dark">
      ...
      <p className="mt-1 text-sm text-ink-light">
        {mode === "signin"
          ? "Sign in to continue your POSSARA journey."
          : mode === "signup"
            ? "Build a profile, save opportunities and connect."
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        {message && <p className="text-sm text-trust-dark">{message}</p>}
      ...
      {mode !== "forgot" && (
        <>
          ...
          <button onClick={handleGoogle} className="w-full rounded-full border border-ink-faint/30 py-2.5 font-medium">
