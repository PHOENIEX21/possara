...
export function JobApply(){
 ...
 <form onSubmit={submit} className="rounded-3xl border border-paper-dim bg-white p-5 shadow-card sm:p-7">
  ...
  <label className="mt-4 block text-sm font-semibold">Cover letter <span className="font-normal text-ink-faint">(optional)</span><textarea rows={5} value={cover} onChange={e=>setCover(e.target.value)} className="mt-1 w-full resize-none rounded-xl border border-black/15 p-3"/></label>
