...
function AdCard({ ad }: { ad: { organization_name: string; title: string; description: string; link: string; image_url: string | null } }) {
  return <a href={ad.link} target="_blank" rel="noreferrer" className="block rounded-2xl border border-paper-dim bg-white p-4 shadow-sm transition hover:border-opportunity hover:shadow-md">
...
function SubmitAdForm() {
  ...
  if(success)return <div className="rounded-xl border border-trust-light bg-trust-light/40 p-5"><p className="text-trust-dark">Submitted for review. Paid activation remains separate from ordinary Business community posts.</p></div>;
...
export function Advertise(){
  ...
  return <div className="page-stack">
    <section className="rounded-[1.75rem] bg-ink p-5 text-white shadow-lg sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.12em] text-white/55">Paid promotion</p>
          <h1 className="mt-1 text-2xl font-bold text-white">Advertise on POSSARA</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">Sponsored campaigns are reviewed and always labelled Sponsored. They remain separate from ordinary community business posts.</p>
        </div>
        <button onClick={()=>setShowForm((value)=>!value)} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-white px-4 py-2 text-sm font-semibold text-ink"><Megaphone size={15}/>{showForm?"Hide form":"Create paid ad"}</button>
      </div>
      {showForm&&<div className="mt-5 max-w-lg text-ink"><SubmitAdForm/></div>}
    </section>

    <section>
      <div className="mb-3 flex items-center gap-2"><Megaphone size={17} className="text-opportunity-dark"/><div><h2 className="font-semibold">Sponsored campaigns</h2><p className="text-xs text-ink-faint">Paid placements only.</p></div></div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {!isLoading&&ads?.length===0&&<p className="col-span-full text-sm text-ink-light">No active sponsored campaigns right now.</p>}
        {ads?.map((ad)=><AdCard key={ad.id} ad={ad}/>)}
      </div>
    </section>

    <section className="rounded-[1.75rem] border border-brand/10 bg-brand-light/20 p-4 sm:p-5">
      <div className="mb-4 flex items-start gap-3">
        <div className="rounded-2xl bg-white p-2.5 text-brand-dark shadow-sm"><Store size={19}/></div>
        <div><p className="eyebrow">Business community</p><h2 className="text-xl font-semibold">Share your work without buying an ad</h2><p className="mt-1 text-sm leading-6 text-ink-light">Products, services, portfolios, small businesses and useful offers can be shared here as ordinary community posts. They are not Sponsored.</p></div>
      </div>
      <PostComposer defaultTopic="business" placeholder="Share your business, service, product, portfolio or offer… Use @username to tag someone."/>
    </section>

    <section>
      <div className="mb-3"><p className="eyebrow">Community business feed</p><h2 className="text-xl font-semibold">What members are building and offering</h2></div>
      {businessLoading&&<div className="feed-skeleton"/>}
      {!businessLoading&&businessPosts?.length===0&&<div className="empty-state"><Store size={26}/><h2>No business posts yet</h2><p>Be the first to share something useful from your business or work.</p></div>}
      <div className="space-y-4">{businessPosts?.map((post)=><PostCard key={post.id} post={post}/>)}</div>
    </section>
  </div>;
}
