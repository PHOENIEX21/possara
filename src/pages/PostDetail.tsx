...
export function PostDetail() {
  ...
  const post = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("posts")
        .select("id,author_id,organization_id,content,media_urls,feeling,music_path,music_title,music_mime_type,created_at,status,deleted_at,shared_from_post_id,profiles(full_name,username,avatar_url,headline,profession),organizations(id,name,slug,logo_url,verified),shared_from_post:posts!posts_shared_from_post_id_fkey(id,author_id,content,media_urls,profiles(full_name,username,avatar_url))")
        .eq("id", id as string)
        .eq("status", "published")
        .is("deleted_at", null)
        .single();
  ...
  const organization = Array.isArray(post.data.organizations) ? post.data.organizations[0] : post.data.organizations;
  const name = organization?.name ?? author?.full_name ?? author?.username ?? "POSSARA member";
  const profilePath = organization ? `/organizations/${organization.slug}` : author?.username ? `/profile/${author.username}` : `/profile/id/${post.data.author_id}`;
    ...
    <div className="mx-auto max-w-2xl space-y-3">
      ...
      <article className="rounded-3xl border border-paper-dim bg-white p-5 shadow-sm">
        <Link to={profilePath} className="flex items-center gap-3">
          {organization ? (organization.logo_url ? <img src={organization.logo_url} alt="" className="h-11 w-11 rounded-xl object-cover"/> : <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-brand-light text-brand-dark"><Building2 size={18}/></div>) : author?.avatar_url ? <img src={author.avatar_url} alt="" className="h-11 w-11 rounded-full object-cover"/> : <div className="flex h-11 w-11 items-center justify-center rounded-full bg-trust-light font-semibold text-trust-dark">{name.charAt(0).toUpperCase()}</div>}
          <div className="min-w-0"><div className="flex items-center gap-1.5"><p className="truncate font-semibold">{name}</p>{organization?.verified&&<OrganizationVerificationBadge compact/>}</div>{organization?<p className="truncate text-xs text-ink-faint">Organization update</p>:(author?.headline || author?.profession) && <p className="truncate text-xs text-ink-faint">{author.headline || author.profession}</p>}<p className="text-[11px] text-ink-faint">{new Date(post.data.created_at).toLocaleString()}</p></div>
        </Link>
        {post.data.feeling&&<p className="mt-3 text-sm font-medium text-brand-dark">{({happy:"😊 Happy",grateful:"🙏 Grateful",excited:"🤩 Excited",celebrating:"🎉 Celebrating",birthday:"🎂 Celebrating a birthday",proud:"🙌 Proud",blessed:"✨ Blessed",motivated:"💪 Motivated"} as Record<string,string>)[post.data.feeling]??post.data.feeling}</p>}{post.data.content && <p className="mt-4 whitespace-pre-wrap text-[15px] leading-7 text-ink"><MentionText text={String(post.data.content).replace(/\\n/g,"\n")}/></p>}
