...
import { ArrowRight, Briefcase, Building2, ExternalLink, GraduationCap, HandHeart, HardHat, HeartPulse, Landmark, Laptop, MapPin, Palette, Search, ShoppingBag } from "lucide-react";
import { supabase } from "../lib/supabase";
import type { Organization } from "../types/database";

const INDUSTRIES = [
  { label: "All", value: "", icon: Building2 },
  { label: "Hotels", value: "Hotels & Hospitality", icon: Building2 },
  { label: "Schools", value: "Education & Schools", icon: GraduationCap },
  ...
  { label: "NGOs", value: "NGO & Foundation", icon: HandHeart },
  { label: "Engineering", value: "Construction & Engineering", icon: HardHat },
  { label: "Media", value: "Media & Creative", icon: Palette },
  { label: "Retail", value: "Retail & Commerce", icon: ShoppingBag },
  { label: "Professional", value: "Professional Services", icon: Briefcase },
] as const;
...
export function Organizations(){
  ...
  const selectedIndustry=INDUSTRIES.find(item=>item.value===industry)?.label??"All";

  return <div className="page-stack">
    <section className="page-hero"><div><p className="eyebrow">Places & organizations</p><h1>Find places to work, learn and grow.</h1><p>You do not need to know a company name first. Browse hotels, schools, health organizations and other workplaces by state, then open their website or see opportunities listed on POSSARA.</p></div></section>

    <section className="rounded-3xl border border-paper-dim bg-white p-4 shadow-sm sm:p-5">
      <label className="relative block w-full"><Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-faint"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search any organization, hotel, school, city or industry…" className="w-full rounded-full border border-ink-faint/30 py-3 pl-10 pr-4 text-sm outline-none focus:border-brand"/></label>
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <button type="button" onClick={()=>{setState("Kwara");setQuery("Ilorin");}} className="rounded-full border border-ink-faint/25 px-3 py-2 text-sm font-medium text-ink-light hover:bg-paper-dim">Explore Ilorin</button>
      </div>

      <div className="mt-5"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-faint">Browse places by type</p><div className="scrollbar-none flex gap-2 overflow-x-auto pb-1">{INDUSTRIES.map(item=>{const Icon=item.icon;const active=industry===item.value;return <button key={item.label} type="button" onClick={()=>setIndustry(item.value)} className={`inline-flex shrink-0 items-center gap-2 rounded-full border px-3.5 py-2 text-sm font-medium transition ${active?"border-brand bg-brand text-white":"border-ink-faint/25 bg-white text-ink-light hover:bg-paper-dim"}`}><Icon size={15}/>{item.label}</button>})}</div></div>
    </section>

    <section><div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">{selectedIndustry}</h2><p className="text-sm text-ink-faint">{state?`Showing ${selectedIndustry.toLowerCase()} in ${state}.`:"Browse across all listed states or narrow the results."}</p></div><span className="text-xs text-ink-faint">{filtered.length} listed</span></div>
      {isLoading&&<div className="feed-skeleton"/>}
      {error&&<p className="text-flag">Couldn't load organizations right now.</p>}
      {!isLoading&&!error&&filtered.length===0&&<div className="rounded-2xl border border-paper-dim bg-white p-6 text-center"><h3 className="font-semibold">No organizations found yet</h3><p className="mt-1 text-sm text-ink-light">Try another type, state or a broader search. POSSARA can keep expanding this directory as organizations are discovered and reviewed.</p></div>}
