...
export function Home(){
  ...
  return <div className="feed-layout">
    <div className="feed-column">
      ...
      <section className="home-community-category-filter" aria-label="Filter home feed">{[...HOME_FILTERS.map(item=>({slug:item.key,label:item.label})),{slug:"jobs",label:"Jobs"},{slug:"scholarships",label:"Scholarships"}].map(item=><button key={item.slug} type="button" className={(item.slug==="for-you"&&!communityCategory&&filter==="for-you")||(item.slug==="insight"&&!communityCategory&&filter==="insight")||communityCategory===item.slug?"active":""} onClick={()=>{if(item.slug==="for-you"||item.slug==="insight"){setCommunityCategory(undefined);setFilter(item.slug as HomeFilter);}else{setCommunityCategory(item.slug);}window.requestAnimationFrame(()=>feedStartRef.current?.scrollIntoView({behavior:"smooth",block:"start"}));}}>{item.label}</button>)}</section>
