  ...
  return <>
    <section className="max-w-3xl overflow-hidden rounded-[2rem] border border-black/[.06] bg-white shadow-card">
      ...
      <div className="relative px-4 pb-6 sm:px-6">
        ...
        {!!profileOrganizations?.length&&<div className="mt-4 rounded-2xl border border-paper-dim bg-paper/55 p-3"><p className="mb-2 text-[11px] font-semibold uppercase tracking-[.12em] text-ink-faint">Organizations</p><div className="flex flex-wrap gap-2">{profileOrganizations.map(org=><div key={org.id} className="flex min-w-0 items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm"><Link to={`/organizations/${org.slug}`} className="flex min-w-0 flex-1 items-center gap-2 transition hover:opacity-80"><span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-paper-dim">{org.logo_url?<img src={org.logo_url} alt="" className="h-full w-full object-cover"/>:<BriefcaseBusiness size={15}/>}</span><span className="min-w-0"><span className="flex items-center gap-1.5"><span className="max-w-[180px] truncate text-sm font-semibold text-ink">{org.name}</span>{org.verified&&<OrganizationVerificationBadge compact/>}</span><span className="block text-[10px] capitalize text-ink-faint">{own?org.member_role:"Organization"}{org.industry?` · ${org.industry}`:""}</span></span></Link>{own&&<button type="button" onClick={()=>{window.localStorage.setItem("possara-active-organization",org.id);window.dispatchEvent(new Event("possara-acting-identity-change"));window.location.assign(`/organizations/${org.slug}`);}} className="shrink-0 rounded-full bg-ink px-3 py-1.5 text-[11px] font-semibold text-white" aria-label={`Switch to ${org.name}`}>Switch</button>}</div>)}</div><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><p className="text-[11px] text-ink-faint">Open an organization page to follow its updates and jobs.</p>{own&&<Link to="/organizations/register" className="text-[11px] font-semibold text-brand-dark hover:underline">Create another organization</Link>}</div></div>}
...
export function Profile(){
  ...
  if(isOwn){if(!userId)return <p className="text-ink-light">Sign in to view your profile.</p>;if(isLoading||!ownProfile)return <p className="text-ink-light">Loading…</p>;if(editing)return <EditOwnProfile onDone={()=>setEditing(false)}/>;return <div className="space-y-5"><ProfileView profile={ownProfile} own onEdit={()=>setEditing(true)} onOwnCoverClick={()=>coverInputRef.current?.click()} onRemoveCover={removeOwnCover} coverUploading={coverBusy}/><div className="grid items-start gap-5 lg:grid-cols-[minmax(250px,.78fr)_minmax(0,1.35fr)]"><aside className="space-y-4 lg:sticky lg:top-20"><div className="rounded-3xl border border-paper-dim bg-white p-5 shadow-sm"><h2 className="font-semibold">Profile</h2><p className="mt-2 text-sm leading-6 text-ink-light">Your introduction, organizations and public details appear above. Use Edit profile to keep them current.</p></div></aside><main className="min-w-0 space-y-4"><PostComposer showHomeTopicPicker placeholder="Share an insight, lesson or useful experience…"/><ProfilePosts profileId={ownProfile.id}/></main></div><input ref={coverInputRef} type="file" accept="image/jpeg,image/png,image/webp" onChange={changeOwnCover} className="hidden"/>{coverStatus&&<p className="max-w-3xl rounded-xl bg-white px-3 py-2 text-sm text-ink-light shadow-sm">{coverStatus}</p>}<Link to="/settings" className="inline-flex rounded-full border border-paper-dim bg-white px-4 py-2 text-sm font-semibold text-brand-dark shadow-sm">Privacy, notifications & account settings</Link></div>;}
...
function PublicProfile({username}:{username:string}){const {data:profile,isLoading,error}=useProfileByUsername(username);if(isLoading)return <p className="text-ink-light">Loading…</p>;if(error||!profile)return <p className="text-flag">This profile couldn&apos;t be found.</p>;return <div className="space-y-5"><ProfileView profile={profile}/><div className="grid items-start gap-5 lg:grid-cols-[minmax(250px,.78fr)_minmax(0,1.35fr)]"><aside className="rounded-3xl border border-paper-dim bg-white p-5 shadow-sm"><h2 className="font-semibold">About</h2><p className="mt-2 text-sm leading-6 text-ink-light">{profile.bio||profile.headline||"Public profile details and activity."}</p></aside><main className="min-w-0"><ProfilePosts profileId={profile.id}/></main></div></div>;}
