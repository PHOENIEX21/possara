...
export function Applications() {
  ...
  const submittedJob=searchParams.get("submitted")==="job";
  ...
  return <div className="page-stack">{submittedJob&&<section className="rounded-2xl bg-trust-light p-4 text-trust-dark"><div className="flex items-start gap-2"><CheckCircle2 size={19} className="mt-0.5 shrink-0"/><div><p className="font-semibold">Application submitted successfully.</p><p className="mt-1 text-sm text-ink-light">Your application is now with the organization. If the role requires CBT, open it below to continue the application flow.</p></div></div></section>}<section className="rounded-3xl bg-ink p-6 text-white sm:p-7"><div className="flex items-center gap-2 text-sm font-medium text-white/65"><BriefcaseBusiness size={16}/> POSSARA+</div><h1 className="mt-2 text-3xl font-semibold">Application Command Center</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">Turn opportunities into action. Track each application, the next thing you need to do, documents/actions in your checklist and approaching deadlines.</p></section><section className="rounded-3xl border border-paper-dim bg-white p-5 shadow-card"><div className="flex items-end justify-between gap-3"><div><p className="eyebrow">Applied inside POSSARA</p><h2 className="text-xl font-bold">Organization job applications</h2></div><Link to="/jobs" className="text-xs font-semibold text-brand-dark">Find jobs</Link></div>{jobsLoading&&<div className="mt-3 feed-skeleton"/>}{jobsError&&<p className="mt-3 text-sm text-flag">Couldn&apos;t load job applications.</p>}<div className="mt-3 space-y-2">{jobApplications?.map((a:any)=>{const statusLabel=({new:"Submitted",shortlisted:"Shortlisted",rejected:"Not selected",hired:"Hired",withdrawn:"Withdrawn"} as Record<string,string>)[a.status]??a.status;return <article key={a.id} className="rounded-2xl bg-paper p-3 sm:p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><Link to={"/jobs/"+a.job_posting_id} className="font-semibold hover:underline">{a.job_postings?.title}</Link><p className="mt-0.5 text-xs text-ink-faint">{a.job_postings?.organizations?.name} · Applied {new Date(a.applied_at).toLocaleDateString()}</p></div><span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold">{statusLabel}</span></div>{a.needs_reapply&&<p className="mt-2 rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-flag">Employer updated this role — review the changes and update your application.</p>}<div className="mt-3 grid grid-cols-2 gap-2"><Link to={"/jobs/"+a.job_posting_id} className="flex min-h-10 items-center justify-center rounded-xl bg-white px-3 py-2 text-xs font-semibold text-ink">View role</Link>{a.job_postings?.requires_cbt?<Link to={"/jobs/"+a.job_posting_id+"/cbt"} className="flex min-h-10 items-center justify-center rounded-xl bg-opportunity-light px-3 py-2 text-xs font-semibold text-opportunity-dark">Open CBT</Link>:<Link to="/notifications" className="flex min-h-10 items-center justify-center rounded-xl bg-brand-light px-3 py-2 text-xs font-semibold text-brand-dark">Hiring updates</Link>}</div></article>})}</div>{!jobsLoading&&!jobsError&&jobApplications?.length===0&&<p className="mt-3 text-sm text-ink-faint">No direct job applications yet.</p>}</section><section className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">{grouped.map((status) => <div key={status.value} className="rounded-2xl border border-black/[.05] bg-white p-3 text-center shadow-sm"><p className="text-xl font-bold">{status.count}</p><p className="mt-0.5 text-[11px] font-medium text-ink-faint">{status.label}</p></div>)}</section>{isLoading && <div className="feed-skeleton"/>}{error && <p className="rounded-2xl bg-red-50 p-4 text-sm text-flag">Couldn&apos;t load your application tracker.</p>}{!isLoading && !error && applications?.length === 0 && <section className="rounded-3xl border border-dashed border-black/10 bg-white p-8 text-center"><Check size={28} className="mx-auto text-ink-faint"/><h2 className="mt-3 text-lg font-semibold">Nothing tracked yet</h2><p className="mx-auto mt-1 max-w-md text-sm leading-6 text-ink-light">Open an opportunity and choose <strong>Track application</strong>. POSSARA will keep the application, next steps and checklist here.</p><Link to="/opportunities" className="mt-4 inline-flex rounded-full bg-ink px-5 py-2.5 text-sm font-semibold text-white">Find opportunities</Link></section>}<div className="space-y-3">{applications?.map((item) => <TrackerCard key={item.id} item={item}/>)}</div></div>;
}
