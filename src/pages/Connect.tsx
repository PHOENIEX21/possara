...
export function Connect(){
  ...
  const {data:skillDirectory}=useSkillDirectory("",60);
  ...
  const filtered=members?.filter(m=>{
    ...
    if(selectedSkill && !(m.skills??[]).some((skill)=>skill.trim().toLowerCase()===selectedSkill.toLowerCase())) return false;
  ...
  return <div className="page-stack">
    ...
    {tab==="people"&&<>
      <div className="scrollbar-none -mx-1 flex gap-2 overflow-x-auto px-1 pb-1"><button onClick={()=>{setProfession(null);setSelectedSkill(null);}} className={`whitespace-nowrap rounded-full border px-3.5 py-1.5 text-sm ${!profession&&!selectedSkill?"border-ink bg-ink text-white":"border-ink-faint/30 text-ink-light"}`}>People to follow</button>{PROFESSIONS.map(p=><button key={p.label} onClick={()=>{setProfession(p.label);setSelectedSkill(null);}} className={`whitespace-nowrap rounded-full border px-3.5 py-1.5 text-sm ${profession===p.label?"border-brand bg-brand text-white":"border-ink-faint/30 text-ink-light"}`}>{p.label}</button>)}</div>
      {!!skillDirectory?.length&&<div><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-faint">Skills members added</p><div className="scrollbar-none -mx-1 flex gap-2 overflow-x-auto px-1 pb-1">{skillDirectory.map(skill=><button key={skill.id} onClick={()=>{setSelectedSkill(current=>current===skill.name?null:skill.name);setProfession(null);}} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold ${selectedSkill===skill.name?"border-trust bg-trust-light text-trust-dark":"border-ink-faint/25 bg-white text-ink-light"}`}>{skill.name}<span className="ml-1 text-[10px] text-ink-faint">· {skill.member_count}</span></button>)}</div></div>}
  ...
  return location.toLowerCase().endsWith(`, ${country.toLowerCase()}`)||location.toLowerCase()===country.toLowerCase()?location:`${location}, ${country}`;
})()}</p>}{matchCount>0&&<p className="mt-1 text-[11px] font-medium text-trust-dark">Relevant to your profile</p>}</Link>
