...
export function InterestingPlaces() {
  ...
  if(!image)return <section className="relative min-h-[300px] overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-slate-950 via-violet-950 to-slate-900 p-6 text-white shadow-xl"><div className="absolute inset-0 opacity-25" style={{backgroundImage:"radial-gradient(circle at 20% 20%, white 0, transparent 28%), radial-gradient(circle at 80% 65%, #a78bfa 0, transparent 30%)"}}/><div className="absolute inset-x-0 bottom-0 p-6"><p className="text-xs font-semibold uppercase tracking-[.16em] text-white/60">Places worth experiencing</p><h2 className="mt-2 text-2xl font-bold">{place.name}</h2><p className="mt-2 text-sm text-white/80">{place.description}</p><p className="mt-3 text-xs font-semibold text-white/60">{place.location}</p></div></section>;
