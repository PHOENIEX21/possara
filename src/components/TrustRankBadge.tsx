...
export function TrustRankBadge({ rank, compact = false }: { rank: MemberTrustRank | null | undefined; compact?: boolean }) {
  ...
  return (
    ...
    >
      {rank.official ? <span aria-hidden="true" className={compact ? "text-[11px] leading-none" : "text-[14px] leading-none"}>🥇</span> : <BadgeCheck size={size} strokeWidth={2.4} aria-hidden="true"/>}
    </span>
  );
}
