...
export function StoriesBar() {
  ...
  async function openGroup(group: AuthorWithStories) {
    const refreshed = await refetchStories();
    const latest = refreshed.data?.find((item) => item.authorId === group.authorId) ?? group;
    setViewingGroup(latest);
  }
  ...
  return <div>
    <div className="scrollbar-none flex gap-3 overflow-x-auto pb-1">
      {userId && <div className="flex shrink-0 flex-col items-center gap-1"><div className="relative h-14 w-14"><button type="button" onClick={() => myGroup ? void openGroup(myGroup) : openComposer()} className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-dashed border-brand/40">{myGroup ? <div className="h-full w-full overflow-hidden rounded-full border-2 border-brand p-0.5">{myGroup.stories[0]?.media_url ? <img src={myGroup.stories[0].media_url} alt="" className="h-full w-full rounded-full object-cover"/> : <div className={`flex h-full w-full items-center justify-center rounded-full text-xs font-semibold text-white ${BACKGROUNDS[myGroup.stories[0]?.background_style ?? "midnight"]}`}>You</div>}</div> : <Plus size={20} className="text-brand" />}</button>{myGroup && <button type="button" onClick={openComposer} className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-brand text-white" aria-label="Add another Moment"><Plus size={14} /></button>}</div><span className="text-[11px] text-ink-light">Your Moment</span></div>}
      {otherGroups.map((group) => <button key={group.authorId} type="button" onClick={() => void openGroup(group)} className="flex shrink-0 flex-col items-center gap-1"><div className="h-14 w-14 rounded-full border-2 border-brand p-0.5"><div className="h-full w-full rounded-full bg-paper p-0.5">{group.author?.avatar_url ? <img src={group.author.avatar_url} alt="" className="h-full w-full rounded-full object-cover" /> : <div className="flex h-full w-full items-center justify-center rounded-full bg-trust-light text-sm font-medium text-trust-dark">{(group.author?.full_name ?? "?").charAt(0).toUpperCase()}</div>}</div></div><span className="max-w-[60px] truncate text-[11px] text-ink-light">{group.author?.full_name?.split(" ")[0] ?? "Member"}</span></button>)}
    </div>
