...
export function StoriesBar() {
  ...
  return <div>
    <div className="scrollbar-none flex gap-3 overflow-x-auto pb-1">
      {userId && <button type="button" onClick={openComposer} className="flex shrink-0 flex-col items-center gap-1" aria-label="Create a new Moment"><span className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-dashed border-brand/45 bg-brand/[.04]"><Plus size={21} className="text-brand" /></span><span className="text-[11px] font-medium text-brand-dark">Add Moment</span></button>}
      {userId && myGroup && <button type="button" onClick={() => setViewingGroup(myGroup)} className="flex shrink-0 flex-col items-center gap-1" aria-label="View your Moments"><span className="h-14 w-14 rounded-full border-2 border-brand p-0.5">{myGroup.stories[0]?.media_url ? <img src={myGroup.stories[0].media_url} alt="" className="h-full w-full rounded-full object-cover"/> : <span className="flex h-full w-full items-center justify-center rounded-full bg-paper text-sm font-medium text-trust-dark">You</span>}</span><span className="text-[11px] text-ink-light">Your Moment</span></button>}
