        ...
        <div className="max-h-[58vh] overflow-y-auto p-2">
          ...
          {people.data?.map((person)=>{
            ...
            return <Link key={person.user_id} to={path} onClick={()=>setPeopleOpen(false)} className="flex items-center gap-3 rounded-2xl p-3 hover:bg-paper">
              ...
              <div className="flex shrink-0 items-center gap-2"><MemberFollowButton targetUserId={person.user_id} compact/><span className="rounded-full bg-paper px-2.5 py-1 text-xs">{reaction?.emoji} {reaction?.label}</span></div>
            </Link>;
          })}
        </div>
      </div>
