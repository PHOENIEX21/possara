...
function CommentThread({ postId }: { postId: string }) {
  ...
  function chooseReply(comment: NonNullable<typeof comments>[number]) {
    const username = comment.profiles?.username ?? null;
    const name = comment.profiles?.full_name ?? username ?? "member";
    setReplyTo({ parentId: comment.id, name, username });
  ...
  function CommentRow({ comment, isReply = false }: { comment: (typeof rows)[number]; isReply?: boolean }) {
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            ...
            {editing?.id === comment.id ? (
              <div className="mt-2">
                <textarea value={editing.text} onChange={(event) => setEditing({ id: comment.id, text: event.target.value })} rows={3} className="w-full resize-none rounded-xl border border-ink-faint/25 bg-white p-2 text-sm outline-none focus:border-brand"/>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3 px-2">
            ...
            {userId && <button type="button" onClick={() => chooseReply(comment)} className="text-[12px] font-semibold text-ink-faint hover:text-ink">Reply</button>}
      ...
      <div className="max-h-[34rem] space-y-4 overflow-y-auto overscroll-contain pr-1">
        ...
        {roots.map((root) => (
          <div key={root.id} className="space-y-2.5">
            ...
            {(repliesByRoot.get(root.id) ?? []).map((reply) => <CommentRow key={reply.id} comment={reply} isReply/> )}
          </div>
        ))}
      </div>
